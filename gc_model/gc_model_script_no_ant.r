...
#join precip to dataframe
standDay5 <- join(standDay4, precipL, by=c("Days"), type="left")

#variables for centering
#air temp
airTmean<- floor(mean(standDay2$Tair))
#thaw depth minimum
TDstart <- aggregate(standDay4$TD, by=list(standDay4$stand), FUN="mean")
...
write.table(standDay5, paste0(saveMdir, "\\out", "\\standDay.csv"), sep=",", row.names=FALSE)
