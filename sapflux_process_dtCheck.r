...
############
#check dT range for data used
#first turn them into dataframes with a sensor column

#########End Transpiration (T) calcs ####################


TdfAllH <- na.omit(data.frame(doy=rep(datDTH$doy,times=8), year=rep(2016,length(datDTH$doy)*8),hour=rep(datDTH$hour,times=8),
						sensor=rep(seq(1,8),each=dim(T.gHf)[1]), stand=rep(1,length(datDTH$doy)*8),
						T=as.vector(data.matrix(T.gHf))))
TdfAllL <- na.omit(data.frame(doy=rep(datDTL$doy,times=16), year=rep(2016,length(datDTL$doy)*16),hour=rep(datDTL$hour,times=16),
						sensor=rep(seq(1,16),each=dim(T.gLf)[1]),stand=rep(2,length(datDTL$doy)*16),
						T=as.vector(data.matrix(T.gLf))))						
						
TdfAllH17 <- na.omit(data.frame(doy=rep(datDTH17$doy,times=16), year=rep(2017,length(datDTH17$doy)*8),hour=rep(datDTH17$hour,times=16),
						sensor=rep(seq(1,16),each=dim(T.gHf17)[1]),stand=rep(1,length(datDTH17$doy)*16),
						T=as.vector(data.matrix(T.gHf17))))
TdfAllL17 <- na.omit(data.frame(doy=rep(datDTL17$doy,times=16), year=rep(2017,length(datDTL17$doy)*16),hour=rep(datDTL17$hour,times=16),
						sensor=rep(seq(1,16),each=dim(T.gLf17)[1]),stand=rep(2,length(datDTL17$doy)*16),
						T=as.vector(data.matrix(T.gLf17))))		

TallC <- rbind(TdfAllH,TdfAllL,TdfAllH17,TdfAllL17)
#turn dT into dataframe too



KAllH <- data.frame(doy=rep(datDTH$doy,times=8), year=rep(2016,length(datDTH$doy)*8),hour=rep(datDTH$hour,times=8),
			sensor=rep(seq(1,8),each=dim(KH)[1]), stand=rep(1,length(datDTH$doy)*8),K=as.vector(data.matrix(KH)))
KAllL <- data.frame(doy=rep(datDTL$doy,times=16), year=rep(2016,length(datDTL$doy)*16),hour=rep(datDTL$hour,times=16),
			sensor=rep(seq(1,16),each=dim(KL)[1]), stand=rep(2,length(datDTL$doy)*16),K=as.vector(data.matrix(KL)))

KAllH17 <- data.frame(doy=rep(datDTH17$doy,times=16), year=rep(2017,length(datDTH17$doy)*16),hour=rep(datDTH17$hour,times=16),
			sensor=rep(seq(1,16),each=dim(KH17)[1]), stand=rep(1,length(datDTH17$doy)*16),K=as.vector(data.matrix(KH17)))
KAllL17 <- data.frame(doy=rep(datDTL17$doy,times=16), year=rep(2017,length(datDTL17$doy)*16),hour=rep(datDTL17$hour,times=16),
			sensor=rep(seq(1,16),each=dim(KL17)[1]), stand=rep(2,length(datDTL17$doy)*16),K=as.vector(data.matrix(KL17)))		

KAllC <- rbind(	KAllH,KAllL,KAllH17,KAllL17)


Kfilt <- join(TallC,KAllC, by=c("doy","year","hour","sensor","stand"), type="inner")
hist(Kfilt$K)
quantile(Kfilt$K,prob=seq(0,1,by=.05))